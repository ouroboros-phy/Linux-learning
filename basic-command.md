...
## 4.基础命令 
### 4.1 ls 
显示指定目录内容，默认当前目录，ls -l 显示详细列表，ls -F显示文件类型信息，可以看到信息中有文件权限，文件所有者，文件大小，文件更改时间，日期，文件名等，ls -a显示所有文件，使用-a可以显示出dot文件，这些文件以.开头shell通配符一般不匹配dot文件,除非明确指定.*。(在通配符使用时,.*可能导致一些问题,因为.*匹配.和..，我们可以使用正则表达式.[^.]*或者.??*排除之)
### 4.2 cp 
用来复制文件
```
$ cp file1 file2
...
### 6.8 sort
sort命令可以将文件内所有行按照字母顺序快速排序，可以使用-n选项按照数字顺序排序那些以数字开头的行，使用-r选项反向排序。
## 7 更改密码和shell
passwd命令乐意更改密码，输入一遍旧密码和两边新密码,chsh更改shell(如ksh 或tcsh)
## 8 环境变量和shell变量
shell中可以保存一些临时变量,shell变量,它们是一些字符值。shell变量可以保存脚本执行中的数据,我们可以用等号=为shell变量复制
如
```
$ STUFF=blah
```
以上命令将blah符值给STUFF我们用$ STUFF来获得该变量的值（用echo $ STUFF）
环境变量与shell雷系,但其不仅仅针对shell而是对于Unix中所有进程都可访问,shell便令只能被当前的shell访问,在shell中运行的命令无法访问，而环境变量可被shell中运行的所有进程访问.
可以用export设置环境变量,如若想将shell变量$ STUFF变成环境变量,可执行以下命令。
```
$ STUFF=blah
$ export STUFF
```
## 9命令路径
PATH是一个特殊的环境变量,它定义了命令路径，它是一个系统目录列表,shell在执行一个命令时,会去这些目录中查找这个命令,如运行ls时，shell会在PATH定义的所有目录里查找ls，若ls出现多个,shell会运行第一个。
```
echo $PATH
```
可以看淡所有的路径组件，它们间用：分隔，我们可以设置PATH变量，为shell查找加入更多路径，如
```
$ PATH = dir:$PATH
```
可以将路径dir加入到PATH的最前面,这样shell会先查找dir，再查找其他路径
```
$ PATH = $PATH:dir
```
这样shell会最后查找dir
PATH使用要十分小心，若不幸删除了所有PATH路径，只需要重启一个新的shell就可以找回。
## 10.特殊字符
对于Linux的一些专业术语，可以去(术语网站)[http://www.catb.org/jargon/html/] 上查看这些术语，对于特殊的字符，我们有
```
* 正则表达式，通用字符
. 当前目录，文件主机名的分隔符
| 管道
...
` 命令替换（可能是左上角的反引号）
'' 半字符串常量
^ 逻辑非，行头（控制键有时在叙述中用^ 表示，如^C 表示ctrl c）
~ 逻辑非，目录快捷方式
# 注释，预处理，替换
[] 范围
{} 声明快，范围
...
```
## 11.命令行编辑
shell中可以用左右箭头编辑命令行，用上下箭头查看历史命令
但用ctrl代替箭头更方便，下面是Unix文本编辑命令
```
ctrl-B 左移光标
ctrl-F 右移光标
ctrl-P 查看上一条命令
...
## 12.文本编辑器
Unix使用纯文本文件保存配置信息，vi和Emacs可掌握其一。vi运行在终端中，使用标准输入输出。
...
```
可以借助关键字查找相关帮助手册。
如果只知道某个功能，但不知道命令命，可以通过关键字查找。
```
$ man -k sort 
```
这可以找到所有与排序相关的命令。
输出结果包括手册的名称、所属章节以及内容的简要描述。
帮助手册的按命令类型组织为多个章节，章节编号出现在章节名后的括号中，在Unix中，章节对应的内容如下：
```
1 用户命令
2 系统命令
3 Unix高级编程库文档
