...
## 4.基础命令 
### 4.1 ls 
显示指定目录内容，默认当前目录，ls -l 显示详细列表，ls -F显示文件类型信息，可以看到信息中有文件权限，文件所有者，文件大小，文件更改时间，日期，文件名等，ls -a显示所有文件，使用-a可以显示出dot文件，这些文件以.开头shell通配符一般不匹配dot文件,除非明确指定.*。(在通配符使用时,.*可能导致一些问题,因为.*匹配.和..，我们可以使用正则表达式.[])
### 4.2 cp 
用来复制文件
```
$ cp file1 file2
